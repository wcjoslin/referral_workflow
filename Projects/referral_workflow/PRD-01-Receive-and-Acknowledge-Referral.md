...
### **Acceptance Criteria**
*   When a message containing a C-CDA file is placed in the mock inbox, a Message Delivery Notification (MDN) is sent back to the sender's address within 5 minutes.
*   The system correctly parses the patient's name and date of birth from the C-CDA header.
*   The system correctly extracts the `Reason for Referral` text from the C-CDA body.
*   If a message does *not* contain a C-CDA file, an MDN is still sent, but an error is logged internally.
...
**Test Plan:**
*   Unit tests for parsing patient name, DOB, and referral reason from sample C-CDA files (e.g., from the `/Referrals - Planned and Completed` examples).
