---
title: 360X Workflow Overview
tags: [architecture, workflow, referral]
aliases: [Workflow, 360X Process]
up: "[[🎯 PROJECT OVERVIEW]]"
---

# 360X Workflow Overview
...
## Open Source C-CDA Repositories
...
## Libraries for Ingestion, Parsing, and Logic
...
- **AI Reasoning (targeted):** Direct `@anthropic-ai/sdk` calls are used only where clinical reasoning is genuinely required:
  1. **PRD-02** — evaluating whether C-CDA content is clinically sufficient for the specialty
  2. **PRD-04** — extracting and structuring clinical text from a signed note into a valid Consult Note C-CDA
  
  All other workflow steps are deterministic and do not invoke the LLM.

---

## Technical Layout for the PoC Application
...
3. **Validation Node (AI):** The agent compares the `Reason for Referral` in the C-CDA against your simulated specialty (e.g., Cardiology).
   - _Error Handling:_ If the C-CDA is missing a "Medication List," the AI identifies this and triggers an "Information Request" message instead of an "Acceptance."
...
- [[../Features/📋 PRD Index|All PRDs]]
- [[Technical Architecture|Architecture Details]]

The [Official 360X Guide](https://www.youtube.com/watch?v=3u_9XU56gP8) provides a deep dive into how DirectTrust manages the trust framework and the technical handshake required for closed-loop referrals.
