...
This PRD addresses the workflow between the patient being scheduled ([[PRD-03 - Schedule Patient|PRD-03]]) and the final consult note being generated ([[PRD-04 - Generate Consult Note|PRD-04]]). While not always required by the 360X standard, providing interim updates can be critical for complex or long-running referral cases.
...
- The application's state machine must be updated to include an `Encounter` state, representing a transition from the `Scheduled` state
- This state indicates that the patient has been seen, and the system is now awaiting a final note to be signed ([[PRD-04 - Generate Consult Note|PRD-04]])
...
- [[PRD-04 - Generate Consult Note|Previous: PRD-04]]
- [[PRD-06 - Close Loop|Next: PRD-06]]
