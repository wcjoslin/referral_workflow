...
export interface ActingUser {
  ...
  active: boolean;               // added in v1.2 for PRD-21 — see below
}
...
**`active` on `ActingUser`, and why `getUser()` does not filter on it.**

The `users.active` column shipped in v1.1, but the type never exposed it, so nothing downstream could
tell an active user from a deactivated one. PRD-21 needs that in two places: it must refuse to assign
work to an inactive user, and it must render an owner who was deactivated *after* being assigned as
inactive rather than as nobody.

`listUsers()` keeps excluding inactive users by default — a picker should not offer them.
`getUser(id)` keeps returning them, deliberately: filtering there would make an owner's display name
resolve to null for a workspace that genuinely has an owner, and the workspace header would then read
"Unassigned" for work somebody owns. A greyed-out name is honest; an empty cell is not.
...
**Version:** 1.2 — `active` added to `ActingUser`, required by PRD-21 and shipping with it. The
column existed from v1.1 but the type never exposed it, so PRD-21's "reject an inactive assignee"
criterion and its deactivated-owner edge case had nothing to read. Records that `getUser()`
deliberately still returns inactive users, so a workspace whose owner has left never reads as
unassigned.
